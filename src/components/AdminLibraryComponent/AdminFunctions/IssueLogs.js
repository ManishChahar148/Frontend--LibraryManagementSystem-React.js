...
export class IssueLogs extends Component {
    constructor(props) {
        super(props)
            ...
            <div>
                ...
                <div key={book.issueTime} className="row">
                    <div className="col-8 mybox">
                        ...
                        <p>Issued to : {book.studentName}</p>
                        <p>Student Roll no. : {book.rollNo}</p>
                        ...
                        <p>Book return Date :
                            {
                            book.return?
                            <span className=" alert-success">{ (new Date(book.returnTime).getDate())+"/"+(new Date(book.returnTime).getMonth()+1)+"/"+(new Date(book.returnTime).getFullYear())} 
                                <p>
                                    {
                                    book.returnedAfterXDays > 10 ?
                                    <span className="alert-danger">Book returned After {book.returnedAfterXDays - 10} extra days</span>
                                    :
                                    <span>returned book on time</span>
                                    }
                                </p>
                            </span>
                            : 
                            <span className="alert-danger">Not yet returned </span> 
                            }
                        </p>
                    </div>
                    <div className="ml-5">
